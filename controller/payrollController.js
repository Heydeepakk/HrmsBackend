...
exports.addSalary = catchAsync(async (req, res, next) => {
    ...
    const wellfare = await req.body.wellfare;
    ...
    con.query(checkExistingQuery, [emp_id], (checkErr, checkResult) => {
        ...
        const insertQuery = `INSERT INTO payroll(emp_id,emp_name,ctc,basic_salary,basic_salary_percentage,hra,hra_percentage,medical,convenience,other_allowance,pf,esi,kra,wellfare,monthly_salary) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`;
        con.query(insertQuery, [emp_id, emp_name, ctc, basic_salary, basic_salary_percentage, hra, hra_percentage, medical, convenience, other_allowance, pf, esi,kra,wellfare,monthly_salary], (err, result) => {
